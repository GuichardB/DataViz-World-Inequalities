...
- [Conclusion](#conclusion)
  ...
  3. [Retours d'expérience](#retours-d'expérience)
...
Nous avons donc créé un dashboard permettant de sélectionner un indicateur du dataset ainsi qu'une période donnée et d'obtenir ainsi rapidement un graph de l'évolution de cette variable au niveau mondial.
Par exemple, on peut ici observer que globalement au niveau mondial, le revenue national n'a cesser d'augmenter dpuis 1980.
...
Par exemple, on peut ici observer la différence entre l'Afrique et l'Europe de l'Est quand à l'évolution de la part des 1% plus riche dans le revenu national. On constaste bien ici que la tendance est à la baisse en Afrique et à la hausse en Europe de l'Est.

![Deuxièmes visu shiny](images/shiny2.png)

Grâce à notre Shiny nous pouvons rapidement nous rendre compte des variables intéressante à étudier et des éventuelles comparaisons à effectuer.
...
Après avoir étudier l'état actuel des inégalités dans le monde, on se pose la question de savoir si il en a toujours été ainsi et si certaines tendances se dégagent depuis toutes ces années.
...
Afin d'avoir une première idée de l'évolution de ces inégalités depuis 1980 nous avons réaliser une représentation de l'évolution de la part dans le revenu national pour chaque tranche de revenu.
On observe que, au niveau mondial il y a autant d'inégalités dans le monde en 1980 qu'en 2019. On observe une légére augmentation dans les années 2000 mais qui est rapidement corrigées.
...
Afin de confirmer cette observation, nous étudions maintenant plus précisement l'évolution de la part de 50% plus pauvres VS les 1% plus riches.
On observe ici que les très riches se sont plus enrichis avant les années 2000 et que les plus pauvres commence a gagner plus depuis 2006.
La question que l'on est ammené à se poser ensuite : Qu'en est-il lorsque l'on regarde plus précisement chaque région ?
...
Ici nous cherchons à étudier l'évolution de la part de 10% plus riches dans le revenu national entre 1980 et 2019.
On observe que dans les régions dîtes "plus développées" tel que l'Europe ou l'Amérique du Nord, cette part est en augmentation constante depuis 1980.
A contrario, dans les régions dîtes "moins développées" tel que l'Afrique ou l'Asie, cette part commence à baisser depuis 2010.
...
Afin d'avoir une analyse plus précise nous allons maintenant observer l'évolution de cette part chez les 50% plus pauvres.
On observe qu'a première vu, dans la plupart des régions les plus pauvres gagnent gagnent de moins en moins. 
Cependant, on remarque que depuis les années 2000, cette décroissance est de moins en moins forte et dans certaines régions comme en Afrique, les plus pauvres commencent a s'enrichir.
On en conclus qu'il y a bien des tendances que se dessinent d'années en années et on a donc cherché un moyen d'étudier plus précisement ces variations.
...
Afin de faire ressortir de nos données le taux de croissance d'e chaque indicateur'un chaque année, on créer une nouvelle colonne qui prend la valeur de l'indicateur voulu une année et lui soustrait sa valeur de l'année précendente. On ajout ensuite le calcul du pourcentage de ce changement par rapport à l'année précendente et on obtient le taux de croissance de cet indicteur.
Ici on test cette approche sur l'évolution du revenu national dans le monde.
On observe notamment qu'il y a eut un boost de croissance à l'an 2000 qui commence a diminuer.
Le crash de 2006, bien que lisser par la courbe moyenne reste bien visible.

Quand est il de cette observation pour chaque région ?
...
Sur cette évolution lissé du taux de croissance du revenu national par région, on oberve que les régions suivent toutes la tendance mondiale. Cependant certaines régions comme les Amériques ou l'Asie décroient plus rapidement comparées à des régions comme l'Europe ou la croissance semble repartir.

Quand est-it de cette évolution de la croissance dans les différentes tranches de la population ?
...
Ici on cherche à étudier l'évolution de la vitesse à laquelle les 1% plus riches s'enrichisse.
On observe que depuis les années 2000 cette vitesse a diminuée mais qu'elle tent peut être à repartir.
...
Sur cette évolution lissé du taux de croissance des 1% plus riches par région, on oberve que les régions suivent toutes la tendance mondiale. Cependant, dans certaines régions comme l'Asie ou l'Océanie la vitesse d'enrichissement des plus riches semble récement s'être réaccelérée.

Quand est-il de cette évolution de la croissance chez les 50% plus pauvres de la population ?
...
Ici on cherche à étudier l'évolution de la vitesse à laquelle les 50% plus pauvres s'enrichisse.
...
Sur cette évolution lissé du taux de croissance des 50% plus pauvres par région, on oberve que les régions suivent toutes la tendance mondiale. Mais ici encore certaines régions comme l'Asie ou l'Océanie voit la vitesse d'enrichissement des plus pauvres s'accelérée récement.
...
De plus, ous avons pu étudier l'évolution de ces inégalités au cours du temps et les éventuelles tendances qui s'en dégageaient.
...
Après avoir suivi l'évolution et les tendances des inégalités à travers le monde, nous nous sommes ensuite demandés pourquoi ces dernières ne font qu'accroitre d'années en années. 
...
ggplot(aes(x=Year))+geom_bar(stat="identity",aes(y=PatrimoineTop1), color="red") +geom_bar(stat="identity", aes(y=PatrimoineBottom50), color="blue") + labs(title="Etats-Unis", x="Année", y = "Part du revenu national (%)") + theme_wsj()+ scale_colour_wsj("colors6")
```
...
#Comparaison de l'evolution de la Parts du revenu national des 1% supérieurs et des 50% inférieurs en France, de 1980 à 2019
final_country_dataset %>% filter(Pays == "France", Year >= 1980) %>%
ggplot(aes(x=Year))+geom_bar(stat="identity",aes(y=PatrimoineTop1), color="red") +geom_bar(stat="identity", aes(y=PatrimoineBottom50), color="blue") + labs(title="France", x="Année", y = "Part du revenu national (%)") + theme_wsj()+ scale_colour_wsj("colors6")
...
#Comparaison de l'evolution de la Parts du revenu national des 1% supérieurs et des 50% inférieurs en France, de 1980 à 2019
final_country_dataset %>% filter(Pays == "France", Year >= 1980) %>%
ggplot(aes(x=Year))+geom_line(aes(y=RevenuNational), color="red") + labs(title="France", x="Année", y = "Part du revenu national (%)") + theme_wsj()+ scale_colour_wsj("colors6")
...
#Comparaison de l'evolution de la Parts du revenu national des 1% supérieurs et des 50% inférieurs en France, de 1980 à 2019
final_country_dataset %>% filter(Pays == "Etats-Unis", Year >= 1980) %>%
ggplot(aes(x=Year))+ geom_line(aes(y=RevenuNational), color="red")  + labs(title="Etats-Unis", x="Année", y = "Part du revenu national (%)") + theme_wsj()+ scale_colour_wsj("colors6")
...
Après cette étude du jeux de données World Inequality Database nous en avons appris beaucoup sur nos données. En effet, lors de notre premier aperçu avec le dataset entre la quantité de données numériques et la complexité des meta-data, nous avions du mal à comprendre le dataset. Mais après s'être plongé dans l'analyse et avoir produit toutes nos visualisations, nous comprenons l'intérets de chaque indicateur économique et de la connaissance qu'on peut en extraire grâce à des graphiques.

La première chose que l'on a appris, ou plutôt confirmé, c'est qu'il y a effectivement beaucoup d'inégalités dans le monde. Egalement sans suprise, ces inégalités sont bien plus présentes dans les régions les moins développées. 
Cependant après avoir analysé l'évolution des divers indicateurs d'inégalité au cours du temps, nous avons appris des choses plus intéressantes. Notemment nous avons observer que, bien que les régions plus pauvres soit plus inégalitaires, la tendance est au changement et les région dîtes plus dévoleppeées voient depuis quelques années leurs inégalités s'accroitrent.

Nous avons ensuite cherché les causes potentielles de ces inégalités et de leurs évolutions.
En analysant l'évolution de la part des capitaux publics face aux capitaux privés nous avons trouvé une cause potentielle. En effet, depuis 1990 la part des capitaux publics ne cesse de diminuer et cela expliquerait le manque d'action des gouvernements par manque de mmoyens.

Enfin, dans notre étude comparative entre deux pays considérés tout les deux comme "riche" ou "développé" la France et les Etats Unis, nous pensions trouver peut de différence. A notre grandre suprise, les USA sont ont vue leurs inégalités croitrent de façon exponentielles tandis qu'en France il y a eut peu de changement.
...
Edgar : "Lors de ce projet (et de cette matière) j'ai appris l'importance de la visualisation et de sa capacité à représenter ou déformer une réalité. Ayant déjà un peu d'expérience en data j'ai orienté notre choix de projet vers un dataset un peu plus compliqué à exploiter et nous l'avons vite regretter. Cependant, à partir de ces données difficiles à comprendre, l'intérêt de la visualisation n'en était que plus marqué et l'analyse s'est révélé plus intéressante que prévue."