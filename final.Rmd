...
```{r message=FALSE}
final_region_dataset %>% filter(Year == "2019", Region!="World") %>%
ggplot(aes(x=NI,y=PTNIT1_in_percent))+geom_point(aes(color=Region))+ labs(title="Corrélation entre hausse de revenu et enrichissement des Top1%", x="Années", y = "Part du revenu national (%)",color = "Classes" )+geom_smooth(method=lm, se=FALSE, linetype="dashed",color="darkred")
```
